...
"""
def get_tether_params(surface, triangle_tags):
    """
    Compute tether lengths from the reference mesh.
    """
    # Extract cartesian positions for each triangle
    triangle_cartesian_positions = np.array([
        surface.points[tri]  # tri is 3 indices
        for tri in triangle_tags
    ])  # shape: (n_triangles, 3, 3)

    # Compute all edge lengths
    edge_lengths = []
    for tri in triangle_cartesian_positions:
        edge_lengths.append(np.linalg.norm(tri[0]-tri[1]))
        edge_lengths.append(np.linalg.norm(tri[1]-tri[2]))
        edge_lengths.append(np.linalg.norm(tri[2]-tri[0]))
    edge_lengths = np.array(edge_lengths)

    global_min_edge = edge_lengths.min()
    global_max_edge = edge_lengths.max()
