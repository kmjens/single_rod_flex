...
def plot_orientation_path_on_S2(timesteps, x, y, z, job):
    ...
    fig.update_layout(
        title="Active Particle Orientation Path on S²",
        scene=dict(
            xaxis_title="X",
            yaxis_title="Y",
            zaxis_title="Z",
    ...
    # Scatter points colored by timestep
    sc = ax.scatter(
        x, y, z,
        c=timesteps,
        cmap="Blues",
    ...
    cbar = fig2.colorbar(sc, ax=ax, shrink=0.7, pad=0.05)
    cbar.set_label("Timestep")

    ax.set_xlabel("X")
    ax.set_ylabel("Y")
    ax.set_zlabel("Z")
    ax.set_title("Active Particle Orientation Path on S²")

    ax.set_box_aspect([1, 1, 1])

    plt.tight_layout()
    plt.savefig(job.fn("orient_path_3D.png"), dpi=300)
    plt.close(fig2)
...
def plot_orientation_on_S2_for_active(phi, theta, job):
    ...
    # Calculate solid angle for each bin
    ...
    fig = go.Figure(data=[
        go.Surface(
            ...
            y=y_grid,
            z=z_grid,
            surfacecolor=solid_angle_distribution.T,
            colorscale=colorscale,
            cmin=0,
            cmax=np.max(solid_angle_distribution),
    ...
    # Plot the spherical heatmap manually using plot_surface
    ax.plot_surface(
        x_grid, y_grid, z_grid,
        rstride=1, cstride=1,
        facecolors=plt.cm.Blues( solid_angle_distribution.T / np.max(solid_angle_distribution) ),
        linewidth=0, antialiased=False
    )

    ax.set_title("Solid Angle Distribution on Sphere")
    ax.set_xlabel("X")
    ax.set_ylabel("Y")
    ax.set_zlabel("Z")
    ax.set_box_aspect([1, 1, 1])

    plt.savefig(job.fn("extra_contrast_3D_orient_dist.png"), dpi=300)
    plt.close(fig2)
...
def plot_v_distrib_on_S2(active_normalized_v, mesh_normalized_v, job):
    ...
    # Second data: Mesh Velocity
    x_grid2, y_grid2, z_grid2, solid_angle_dist2 = plot_solid_angle_dist(mesh_normalized_v)
    colorscale_red = matplotlib_to_plotly(plt.get_cmap('Reds'))

    # Create subplot with two 3D scenes side by side (Plotly)
    fig = make_subplots(
        rows=1, cols=2,
        specs=[[{'type': 'scene'}, {'type': 'scene'}]],
        subplot_titles=("Active Particle Velocity", "Mesh Velocity")
    )

    # First surface (Plotly)
    fig.add_trace(go.Surface(
    ...
    # Second surface (Plotly)
    fig.add_trace(go.Surface(
        x=x_grid2, y=y_grid2, z=z_grid2,
        surfacecolor=solid_angle_dist2.T,
    ...
    # Left subplot: active velocities
    ax1 = fig2.add_subplot(121, projection='3d')
    ax1.plot_surface(
        x_grid1, y_grid1, z_grid1,
        rstride=1, cstride=1,
        facecolors=plt.cm.Blues(solid_angle_dist1.T / np.max(solid_angle_dist1)),
        linewidth=0, antialiased=False
    )
    ax1.set_title("Active Particle Velocity")
    ax1.set_xlabel("X")
    ax1.set_ylabel("Y")
    ax1.set_zlabel("Z")
    ax1.set_box_aspect([1, 1, 1])

    # Right subplot: mesh velocities
    ax2 = fig2.add_subplot(122, projection='3d')
    ax2.plot_surface(
        x_grid2, y_grid2, z_grid2,
        rstride=1, cstride=1,
        facecolors=plt.cm.Reds(solid_angle_dist2.T / np.max(solid_angle_dist2)),
        linewidth=0, antialiased=False
    )
    ax2.set_title("Mesh Velocity")
    ax2.set_xlabel("X")
    ax2.set_ylabel("Y")
    ax2.set_zlabel("Z")
    ...
    plt.tight_layout()
    plt.savefig(job.fn("plot_BOTH_v_distribs_on_S2.png"), dpi=300)
    plt.close(fig2)


def plot_COM_velocity(timesteps, active_com_v_norms, mesh_com_v_norms, speed_conv_mm, step_to_sec, job):
    fig, ax = plt.subplots(figsize=(10, 6))
    ax.set_facecolor('white')
    ax.plot(timesteps, active_com_v_norms, label="Active rod", color='blue',alpha=.8)
    ax.plot(timesteps, mesh_com_v_norms, label="Mesh COM", color='red',alpha=.8)
...
def plot_COM_velocity_FFT(freq, active_v_fft, mesh_v_fft, fft_xlim_max, job):
    ...
    ax.plot(freq[pos_mask], np.abs(mesh_v_fft[pos_mask]), label='FFT of mesh velocity', color='red', alpha=0.8,linestyle='-')
    ...
    ax.legend()
    plt.title('FFT of Magnitudes of Rod and Mesh C.O.M. Velocity')
    plt.tight_layout()

    fig.savefig(job.fn('FFT_velocity_mags_BOTH_w_time.png'), dpi=300, bbox_inches='tight', facecolor='white')
    plt.close()
...
def plot_z_position_data(active_z_exp, mesh_z_exp, z_diff, timesteps, lim, step_to_sec, job):
    fig, ax = plt.subplots(figsize=(10, 6))
    ax.set_facecolor('white')
    ax.plot(timesteps, active_z_exp, label="Active rod", color='blue', alpha=0.8)
    ax.plot(timesteps, mesh_z_exp, label="Mesh center of mass", color='red', alpha = 0.8)
    ax.set_xlabel("Simulation Timestep")
...
def plot_z_position_FFT(freq, mesh_z_fft, active_z_fft, diff_z_fft, fft_xlim_max, job):
    # Only take the positive half of the spectrum
    pos_mask = freq > 0
    ...
    ax.plot(freq[pos_mask], np.abs(mesh_z_fft[pos_mask]), label='mesh z-position', alpha=0.8, color='red',linestyle='-')
    ax.plot(freq[pos_mask], np.abs(active_z_fft[pos_mask]), label='active z-position', alpha=0.8, color='blue',linestyle='--')
    ax.plot(freq[pos_mask], np.abs(diff_z_fft[pos_mask]), label='z-position difference',alpha=0.7, color='grey',linestyle='-')
    ax.set_ylabel('Amplitude')
    ax.set_xlabel('Frequency (Hz)')
    ax.set_xlim([0,fft_xlim_max])
    ax.legend()
...
def plot_distances(timesteps, mesh_total_distances_um, active_total_distances_um, step_to_sec, active_linReg, mesh_linReg, disp_corr, job):
    fig, ax = plt.subplots(figsize=(10, 6))
    ax.set_facecolor('white')
    ...
    ax.text(0.05, 0.8, f"Slope (Mesh COM): {mesh_linReg['slope']:.2e}"+r" ($\mu$m/sec)"+f"\nR² (Mesh COM): {mesh_linReg['r_squared']:.2f}\nStd Err (Mesh COM): {mesh_linReg['std_err']:.2e}",
...
def plot_displacement(timesteps, disp_active_um, disp_mesh_um, step_to_sec, disp_corr, job):
    fig, ax = plt.subplots(figsize=(10, 6))
    ax.set_facecolor('white')
...
def plot_displacement_FTT(timesteps, freq, mesh_disp_fft, active_disp_fft, step_to_sec, fft_xlim_max, job):
    # Only take the positive half of the spectrum
    pos_mask = freq > 0
    ...
    ax.plot(freq[pos_mask], np.abs(mesh_disp_fft[pos_mask]), label='mesh', alpha=0.8, color='red',linestyle='-')
